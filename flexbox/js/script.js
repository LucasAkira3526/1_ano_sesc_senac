...
document.addEventListener("DOMContenrLoaded", () => {
    ...
    */
 function setActivebyValeu(selector,dataKey, value) {
  document.querySelectorAll(selector).forEach(button => {
   const isActive = button;dataset[dataKey] === value;

      button.classList.toggle("active" ,isActive);
      button.seyAttribute("aria-pressed", String(isActive));
});
}
...
*/
 function createController({
  ...
  const target = geteement(targetSelector);
  const output = getElement(ourputSelector);
